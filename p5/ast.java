...
//     public void typeCheck() {}  // For statements and declarations
//     public Type typeCheckExpr() { return new ErrorType();
//  }  // For expressions
...
class StmtListNode extends ASTnode {
    ...
    public void typeCheck(Type fnRetType) {
        for (StmtNode node : myStmts) {
            node.typeCheck(fnRetType);
        }
    }
}
...
abstract class StmtNode extends ASTnode {
    ...
    public void typeCheck(Type fnRetType) {}  // Overridden in subclasses
}
...
class PostIncStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        if (!myExp.typeCheck().isIntegerType() && !myExp.typeCheck().isErrorType())
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Arithmetic operator with non-integer operand");
    }
}
...
class IfStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        if (!myExp.typeCheck().isBooleanType() && !myExp.typeCheck().isErrorType())
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Non-boolean expression in if condition");
        myStmtList.typeCheck(fnType);
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        if (!myExp.typeCheck().isBooleanType() && !myExp.typeCheck().isErrorType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Non-boolean expression in while condition");
        }
        myStmtList.typeCheck(fnType);
    }
...
class ReadStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        ...
        if (type.isFuncType())
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Attempt to read function name");
...
class WriteStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        Type type = myExp.typeCheck();
        if (type.isFuncType())
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Attempt to write function name");
        if (type.isStructDefType())
...
class ReturnStmtNode extends StmtNode {
    ...
    public void typeCheck(Type fnType) {
        if (myExp == null) {
            ...
            Type type = myExp.typeCheck();
            if (!fnType.isVoidType()) {
                if (!fnType.isErrorType() && !type.isErrorType() && !fnType.equals(type))
                    ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Bad return value type");
            } else
...
class StructAccessExpNode extends ExpNode {
    ...
    @Override
    public Type typeCheck() {
        if (badAccess) return new ErrorType();
    
        Sym sym = myId.sym();
        if (sym == null) {
            ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Invalid struct field access");
            return new ErrorType();
        }
    
        Type type = sym.getType();
        if (type == null) {
            ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Unknown type for struct field");
            return new ErrorType();
        }
...
class AssignExpNode extends ExpNode {
    ...
    @Override
    public int charNum() {
        return myLhs.charNum(); // or choose based on your semantics
    }

    @Override
    public int lineNum() {
        return myLhs.lineNum(); // or use a better location if you have one
    }
    ...
    public Type typeCheck() {
        ...
        if (lhsType instanceof VoidType) {
            ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Cannot assign to void");
            return new ErrorType();
        }
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        List<Type> formalTypes = funcSym.getParamTypes();
        List<ExpNode> actuals = myExpList.getExpList();
    
        if (formalTypes.size() != actuals.size()) {
            ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Function call with wrong number of args");
            return new ErrorType();
        }
    
        for (int i = 0; i < actuals.size(); i++) {
            Type actualType = actuals.get(i).typeCheck();
            Type formalType = formalTypes.get(i);
    
            if (!actualType.equals(formalType) && !actualType.isErrorType()) {
                ErrMsg.fatal(actuals.get(i).lineNum(), actuals.get(i).charNum(),
...
class NotNode extends UnaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!type.isErrorType() && !type.isBooleanType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Logical operator with non-boolean operand");
            return new ErrorType();
        }
        return type.isErrorType() ? new ErrorType() : new BooleanType();
    }
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public Type typeCheck() {
        ...
        return type.isErrorType() ? new ErrorType() : new IntegerType();
    }
...
class AndNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!type1.isErrorType() && !type1.isBooleanType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Logical operator with non-boolean operand");
            error = true;
        }

        if (!type2.isErrorType() && !type2.isBooleanType()) {
            ErrMsg.fatal(myExp2.lineNum(), myExp2.charNum(), "Logical operator with non-boolean operand");
            error = true;
        }

        if (type1.isErrorType() || type2.isErrorType()) error = true;
...
class PlusNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!type1.isErrorType() && !type1.isIntegerType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Arithmetic operator with non-integer operand");
            error = true;
        }

        if (!type2.isErrorType() && !type2.isIntegerType()) {
            ErrMsg.fatal(myExp2.lineNum(), myExp2.charNum(), "Arithmetic operator with non-integer operand");
            error = true;
        }
    ...
    public Type typeCheck() {
        ...
        if (!type1.isErrorType() && !type2.isErrorType()) {
            ...
            if (type1.isFuncType() && type2.isFuncType()) {
                ErrMsg.fatal(lineNum(), charNum(), "Equality operator applied to function names");
                return new ErrorType();
            }
            if (type1.isVoidType() && type2.isVoidType()) {
                ErrMsg.fatal(lineNum(), charNum(), "Equality operator applied to void function calls");
                return new ErrorType();
            }
...
class GreaterNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!type1.isErrorType() && !type1.isIntegerType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Relational operator with non-integer operand");
            error = true;
        }

        if (!type2.isErrorType() && !type2.isIntegerType()) {
            ErrMsg.fatal(myExp2.lineNum(), myExp2.charNum(), "Relational operator with non-integer operand");
            error = true;
        }
